 ...
 * Plugin Name: Recapture for Restrict Content Pro
...
if (!class_exists('Recapture')) {
    ...
    include(dirname(__FILE__) . '/platforms/class-rcp.php');

    define('RECAPTURE_VERSION', '1.0.18');
    define('RECAPTURE_TEXT_DOMAIN', 'recapture');
    define('RECAPTURE_TARGET_PLATFORM', 'Restrict Content Pro');
    ...
    class Recapture
    {
        ...
        public function __construct()
        {
            ...
            // setup the platform
            if (RecaptureRCP::is_ready()) {
                $this->platform = new RecaptureRCP();
            } else {
            ...
            // Start member export
            add_action('admin_post_recapture_export_members',
                [$this, 'start_members_export']);
        ...
        public function start_members_export() {
            update_option('recapture_is_exporting', true);
            wp_schedule_single_event(time(), 'recapture_run_export');
            wp_safe_redirect('admin.php?page=recapture');
        }
        ...
        public function register_settings()
        {
            ...
            register_setting('recapture-rcp-settings-group',
                'recapture_rcp_exclude_renewal_carts');
        }
            ...
            <div class="wrap">
                ...
                <form method="post" action="<?php echo esc_html(admin_url('admin-post.php')); ?>" target="_blank">
                    <?php if (RecaptureUtils::is_connected()) { ?>
                        ...
                        <h3 class="recapture-disconnect">Export Memberships to Recapture</h3>
                        <p>
                            <?php esc_html_e('Export existing memberships to Recapture so you can send Winbacks campaigns when a membership expires.', RECAPTURE_TEXT_DOMAIN) ?>
                        </p>
                        <p>
                            <a href="<?= esc_html(wp_nonce_url(admin_url('admin-post.php?action=recapture_export_members'), 'export_rpc_memberships')) ?>" class="button">
                                <?php esc_html_e('Start Export', RECAPTURE_TEXT_DOMAIN) ?>
                            </a>
                        </p>
                ...
                <?php if (RecaptureUtils::is_connected()) { ?>
                    ...
                    <h3 class="recapture-disconnect">
                        <?= __('Advanced Settings', RECAPTURE_TEXT_DOMAIN) ?>
                    </h3>
                    <?php settings_fields('recapture-rcp-settings-group'); ?>
                    <?php do_settings_sections('recapture-rcp-settings-group'); ?>
                    <p>
                        <input
                            type="checkbox"
                            name="recapture_rcp_exclude_renewal_carts"
                            id="recapture_rcp_exclude_renewal_carts"
                            value="1"
                            <?php checked("1", get_option("recapture_rcp_exclude_renewal_carts"), true); ?>
                        />
                        <label for="recapture_rcp_exclude_renewal_carts">
                            <?= __("Don't send abandoned cart emails for subscription renewals", RECAPTURE_TEXT_DOMAIN) ?>
                        </label>
                    </p>
    ...
    function recapture_plugin_activated($plugin) {
        $is_ready = RecaptureRCP::is_ready();
